# 안녕하세요, 최수환입니다 👋
Python 기반 AI/ML 엔지니어입니다.
데이터와 모델을 연결해 실제 서비스에서 동작하는 AI 기능으로 구현하는 데 강점이 있습니다.

## 🧭 About

문제 정의 → 데이터 설계 → 모델링 → API/서비스화까지 End-to-End로 해결합니다.
성능 지표뿐 아니라 사용 환경의 변동·예측 안정성까지 고려한 모델 개선을 추구합니다.
자동화·문서화·재현성을 바탕으로 팀이 확장 가능한 AI 개발 방식을 지향합니다.
관심 분야: ML 모델 최적화, RAG 기반 검색·추천, 사용자 데이터 활용 AI 서비스 고도화
